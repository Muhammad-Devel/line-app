...
export const CustomerHome = () => {
  ...
  useEffect(() => {
    // Endpoint 4: Mijoz uchun mahsulotlar
...
//     const res = await api.post('/client/orders', orderData);
    
//     // Agar bu Hybrid biznes bo'lsa, avtomatik navbatga ham qo'shamiz (Endpoint 8)
//     await api.post('/client/queue', { 
//        serviceId: items[0].id, // Birinchi mahsulotni xizmat sifatida olish
//        orderId: res.data.order._id 
//     });

//     clearCart();
//     navigate('/c/my-queue'); // Navbat statusiga o'tish
//   } catch (err) {
    ...
    <div className="min-h-screen bg-slate-50 pb-32">
        ...
        {['Hammasi', 'Ovqatlar', 'Ichimliklar', 'Shirinliklar'].map(cat => (
          <button
            ...
            className={`px-6 py-3 rounded-2xl font-bold whitespace-nowrap transition-all ${
              activeCategory === cat ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'bg-white text-slate-400'
            }`}
          >
            {cat}
          </button>
      ...
      <div className="grid grid-cols-1 gap-4 px-6">
        {products.map((product: any) => (
          <div key={product._id} className="bg-white p-4 rounded-[32px] flex items-center gap-4 shadow-sm border border-transparent active:scale-95 transition-all">
      ...
      {items.length > 0 && (
        <div className="fixed bottom-8 left-6 right-6 animate-in slide-in-from-bottom-10 duration-500">
          <Button 
            ...
            onClick={() => {/* To'lov/Savat oynasini ochish */}}
          >
            <div className="flex items-center gap-3">
              <span className="bg-white/20 px-3 py-1 rounded-lg text-sm font-black">{items.length}</span>
