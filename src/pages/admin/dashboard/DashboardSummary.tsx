  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      {cards.map((card, idx) => (
        <Card
          key={idx}
          className="p-6 border-none shadow-sm bg-white rounded-[32px] hover:shadow-md transition-shadow"
        >
          <div className="flex justify-between items-start">
            ...
            <span
              className={`text-[10px] font-black px-2 py-1 rounded-lg ${card.bg} ${card.color}`}
            >
              {card.change}
            </span>
          </div>
