...
function Header({}: Props) {
  ...
  const filteredNavItems = ADMIN_NAV_ITEMS.filter(item => 
      !item.roles || item.roles.includes(businessType)
    );

  return <header className="h-20 bg-white border-b border-slate-100 px-8 flex items-center justify-between shrink-0">
            <div>
              <h2 className="text-sm font-bold text-slate-400 uppercase tracking-widest">
                {filteredNavItems.find(i => location.pathname.includes(i.path))?.title || 'Boshqaruv'}
              </h2>
            </div>
            ...
            <div className="flex items-center gap-4">
               ...
               <div className="flex items-center gap-3">
                  <div className="text-right hidden sm:block">
                    ...
                    <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase">{businessType}</p>
                  </div>
                  <button 
                    onClick={logout}
